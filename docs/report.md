...
git checkout report
```
После создадим вручную папку docs с пустым файлом report.md \
Потом: 
```
git add docs/report.md
```
А далее запишем туда весь отчёт и оформим добавление по частям
```
git add -p docs/report.md
```
![](11_1.bmp) \
Выберим поле редактирования (edit) - e \
![](11_2.bmp) \
Далее редактируем и вылазят предварительные результаты \
![](11_3.bmp) \
После редактирования в текстовом редакторе, где мы оставили только ту часть, которую хотим коммитить, мы коммитим. \
![](11_4.bmp) \
И так мы делаем 11 раз
